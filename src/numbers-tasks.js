...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  if (Math.atan2(y1, x1) === Math.atan2(y2, x2)) return 0;
  const angle1 = Math.abs(Math.atan2(y1, x1));
  const angle2 = Math.abs(Math.atan2(y2, x2));
...
function getSumOfDigits(num) {
  function sumDigits(n) {
    if (n === 0) return 0;
    return (n % 10) + sumDigits(parseInt(n / 10, 10));
  }
...
function getRandomInteger(min, max) {
  ...
  return Math.floor(Math.random() * (Math.floor(max) - min1) + min1);
}
